...
{{-- 
    ...
    <form id="deleteForm" action="{{ route('admin.products.destroy', $product->id) }}" method="POST">
    @csrf
    @method('DELETE')
    <button type="submit" onclick="reloadPage({{ $products->currentPage() }})" class="bg-red-500 text-white rounded py-2 px-4 hover:bg-red-700">Delete Product</button>
    <a href="{{ route('admin.dashboard') }}" class="text-black ml-4">Cancel</a>
</form>
@endsection
